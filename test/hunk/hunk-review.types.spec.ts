...
describe('hunk-review.types', () => {
  it('should model a diff line-map keyed by file path', () => {
    const map: DiffLineMap = {
      files: [
        { path: 'src/a.ts', newLines: new Set([10, 11, 12]), changeKind: 'modified', binary: false },
      ],
    };
  ...
  it('should model the hunk agent-context schema', () => {
    const ctx: AgentContext = {
      ...
      summary: 'overview',
      files: [
        { path: 'src/a.ts', annotations: [{ newRange: [10, 12], summary: 'note', author: 'tawtui-review' }] },
      ],
    };
  ...
  it('should model the persisted hunk-review record', () => {
    const rec: HunkReviewRecord = {
      prKey: 'octo/repo#pr-7', repoOwner: 'octo', repoName: 'repo', prNumber: 7,
      worktreePath: '/tmp/wt', port: 41001, sdkSessionId: 'abc-123', status: 'ready',
      createdAt: new Date().toISOString(),
    };
    const out: ReviewOutput = {
      body: { summary: 's', unanchoredFindings: [], unanchoredCount: 0 },
      anchoredFindings: [], agentContextPath: '/tmp/findings.json',
    };
